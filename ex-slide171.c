...
int menu(void)
{
	...
	do {
		...
		printf("\t 5. Recuperar lista do arquivo");
		printf("\t 6. Sair\n");
...
void gravaarq (void) {
	...
	for (i = 0; i<MAX; i++)
		if (info[i].nome[0] != '\0') {
			...
			fprintf(fp, "\t Nome: %s", info[i].nome);
			fprintf(fp, "\t Rua: %s", info[i].rua);
			fprintf(fp, "\t Cidade: %s", info[i].cidade);
			fprintf(fp, "\t Estado: %s\n", info[i].estado);
			fprintf(fp, "\t CEP: %lu\n", info[i].cep);
		}
	fclose(fp);
